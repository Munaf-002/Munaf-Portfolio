...
const ContactForm = () => {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      await fetch("https://formsubmit.co/itsasif009@gmail.com", {
        method: "POST",
        headers: {
  ...
  const handleBlur2 = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    if (e.target.value === '') setIsFocused2(false);
  };
  const handleBlur3 = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    if (e.target.value === '') setIsFocused3(false);
  };
  const handleBlur4 = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    if (e.target.value === '') setIsFocused4(false);
  };
